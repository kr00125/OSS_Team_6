...
class Particle {                         // Particle Class (ipja)
  ...
  void move() {                          // Particle move function
    ...
    vel.mult(speed+dB/10);                     // vel scalar value is 'speed' 
    loc.add(vel);                        // loc<- loc + vel
 
    if (PVector.dist(loc, ploc) < RAND_SPEED_E+dB/10) {   // calculate the distance between two points, loc and ploc if loc <-> ploc < 2
